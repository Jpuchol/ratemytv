		  ...
		  while (page.links[i].text().eql?(" Page suivante")==false && page.links[i].text().eql?(" Page précédente") == false)
		  	if(page.links[i].text().eql?(" ") == false)
			    infos = page.links[i].text().split("Court métrage")[0]
              	...
              	puts infos[0]
              	puts 'http://www.canalplus.fr/c-cinema/pid6268-les-films-sur-canal.html?page='+j.to_s+'&onglet=1'
			    ...
			    if (film_retrieve.links[link] == nil) 
			      ...
			      if(name.eql?("")==false && name != nil)
			      	Film.film!(name.to_s,notePresse,noteSpectateurs,link_allocine.to_s,senscritique)
			      end
			    end
			end
		    i=i+1
